defmodule Entice.Utils.ETSSupervisor do
  ...
  """
  def start_link(name, opts \\ []), do: GenServer.start_link(__MODULE__, name, opts)
  ...
  """
  def start(name, id, args \\ []), do: GenServer.call(name, {:start, id, args})

  @doc """
  Kills a worker if it exists. Does nothing otherwise.
  """
  def terminate(name, id), do: GenServer.call(name, {:terminate, id})
...
defmodule Entice.Utils.ETSSupervisor.Spawner do
  ...
  def start_link(spawned, opts \\ []), do: Supervisor.start_link(__MODULE__, spawned, opts)

  def start_child(spawner, args), do: Supervisor.start_child(spawner, args)

  def terminate_child(spawner, pid), do: Supervisor.terminate_child(spawner, pid)
...
defmodule Entice.Utils.ETSSupervisor.Supervisor do
  ...
  def start_link(name, spawned), do: Supervisor.start_link(__MODULE__, {name, spawned})
