defmodule Entice.Entity.Behaviour do
  ...
  defmacro __using__(_) do
    quote do
      ...
      def fetch_attribute!(%Entity{attributes: attrs}, attribute_type)
          when is_atom(attribute_type),
          do: Map.fetch!(attrs, attribute_type)

      def get_attribute(%Entity{attributes: attrs}, attribute_type) when is_atom(attribute_type),
        do: Map.get(attrs, attribute_type)

      def take_attributes(%Entity{attributes: attrs}, attribute_types)
          when is_list(attribute_types),
          do: Map.take(attrs, attribute_types)

      def put_attribute(
            %Entity{attributes: attrs} = entity,
            %{__struct__: attribute_type} = attribute
          ),
      ...
      def remove_attribute(%Entity{attributes: attrs} = entity, attribute_type)
          when is_atom(attribute_type),
          do: %Entity{entity | attributes: Map.delete(attrs, attribute_type)}

      def attribute_transaction(%Entity{attributes: attrs} = entity, modifier)
          when is_function(modifier, 1),
